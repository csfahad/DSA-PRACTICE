...
// Intuition: For each number between 1 to N, we will try to count the occurrence in the given 
// array using linear search. And the element with occurrence as 2 will be the repeating number 
// and the number with 0 occurrences will be the missing number.
...
// Intuition: Instead of counting the occurrences every time, using the hashing technique, 
// we will store the frequency of each element between 1 to N. Now, the element with frequency 2 will be 
// the repeating number and the element with frequency 0 will be the missing number.
...
// Intuition: The idea is to convert the given problem into mathematical equations. 
