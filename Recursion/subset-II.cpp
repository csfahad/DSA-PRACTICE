...
// CODE STUDIO: https://www.codingninjas.com/studio/problems/unique-subsets_3625236
...
// insert every combination of average length k in a set of size x. After this, we have to convert 
// the set of combinations back into a list of list /vector of vectors which takes more time.
...
// Time Complexity: O(2^n) for generating every subset and O(k)  to insert every subset 
// in another data structure if the average length of every subset is k. Overall O(k * 2^n).
